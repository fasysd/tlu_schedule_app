...
class CourseDetailPage extends StatelessWidget {
  ...
  Widget _buildInfoRow(BuildContext context, {required String label, required String value, Widget? trailing}) {
    return Padding(
  ...
  Widget build(BuildContext context) {
    if (schedules.isEmpty) {
      return Scaffold(appBar: AppBar(title: Text("Lỗi")), body: Center(child: Text("Không tìm thấy lịch học cho học phần này.")));
    }
    ...
    final attendanceRate = (course.totalPeriods > 0) ? (taughtPeriods / course.totalPeriods * 100).toStringAsFixed(0) : 0;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Chi tiết học phần', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        leading: IconButton(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow(context, label: 'Mã học phần:', value: course.courseCode),
            _buildInfoRow(context, label: 'Tên học phần:', value: course.subjectName),
            _buildInfoRow(context, label: 'Lớp học phần:', value: course.className),
            _buildInfoRow(context, label: 'Dạng môn:', value: course.courseType),
            _buildInfoRow(context, label: 'Số tiết:', value: '${course.totalPeriods} (${course.credits} tín chỉ)'),
            _buildInfoRow(context, label: 'Giảng viên:', value: '${user.fullName} - ${user.id.toUpperCase()}'),
            _buildInfoRow(context, label: 'Thời gian:', value: '${DateFormat('dd/MM/yyyy').format(schedules.first.date)} -> ${DateFormat('dd/MM/yyyy').format(schedules.last.date)}'),
            _buildInfoRow(
              context,
              label: 'Sinh viên:',
              ...
              trailing: SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: const Text('Danh sách', style: TextStyle(fontSize: 12, color: Colors.white)),
                ),
              ),
            ),
            ...
            Table(
              ...
              children: [
                const TableRow(
                  children: [
                    Text('Thời gian', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                    Text('Phòng học', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                  ],
                ),
                ...scheduleInfo.values.map((info) => TableRow(
                  children: [
                    Padding(padding: const EdgeInsets.symmetric(vertical: 4.0), child: Text(info['time']!, style: const TextStyle(fontSize: 13))),
                    Padding(padding: const EdgeInsets.symmetric(vertical: 4.0), child: Text(info['room']!, style: const TextStyle(fontSize: 13))),
                  ],
                )),
              ],
            ),
            ...
            _buildInfoRow(context, label: 'Số tiết đã dạy:', value: '$taughtPeriods'),
            _buildInfoRow(context, label: 'Số tiết đã nghỉ:', value: '$missedPeriods'),
